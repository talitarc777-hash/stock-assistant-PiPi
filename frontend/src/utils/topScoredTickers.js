...
export function rankTopScoredTickers(decisions = [], watchlistRows = [], limit = 10) {
  ...
  for (const item of decisions) {
    ...
    const candidate = {
      ...
      ticker_name_en: item?.ticker_name_en || item?.metadata?.ticker_name_en,
      ticker_name_zh: item?.ticker_name_zh || item?.metadata?.ticker_name_zh,
      company_name: item?.company_name || item?.metadata?.company_name,
      company_name_zh: item?.company_name_zh || item?.metadata?.company_name_zh,
      security_name: item?.security_name || item?.metadata?.security_name,
      security_name_zh: item?.security_name_zh || item?.metadata?.security_name_zh,
      score,
  ...
  for (const item of watchlistRows) {
    ...
    latestDecisions.set(ticker, {
      ...
      ticker_name_en: item?.ticker_name_en,
      ticker_name_zh: item?.ticker_name_zh,
      company_name: item?.company_name,
      company_name_zh: item?.company_name_zh,
      security_name: item?.security_name,
      security_name_zh: item?.security_name_zh,
      score,
